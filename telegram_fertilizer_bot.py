...
precipitation = region_precipitation.get(text, 'Невідомо')

await state.update_data(moisture=zone, precipitation=precipitation)

await message.answer(
    f'📍 Ви обрали {text}.\n'
    f'💧 Зона зволоження: {zone}.\n'
    f'🌧️ Середньорічні опади: {precipitation} мм.\n'
    '📊 Вкажіть очікувану врожайність (т/га):'
)

    # Середньорічна кількість опадів (мм) по регіонах України
region_precipitation = {
    'Одеська': 400, 'Миколаївська': 450, 'Херсонська': 350,
    'Запорізька': 420, 'Донецька': 430, 'Луганська': 390,
    'Дніпропетровська': 460, 'Кіровоградська': 480,
    'Харківська': 500, 'Полтавська': 520, 'Черкаська': 530,
    'Київська': 600, 'Тернопільська': 650, 'Хмельницька': 680,
    'Вінницька': 700, 'Львівська': 800, 'Івано-Франківська': 850,
    'Закарпатська': 900, 'Чернівецька': 750, 'Волинська': 780,
    'Рівненська': 760, 'Житомирська': 710, 'Чернігівська': 690,
    'Сумська': 670
}
...
async def compute_recommendations(message: types.Message, state: FSMContext):
    ...
    if crop not in base_requirements:
        ...
        def adjust_fertilizer_rates(n, p, k, precipitation):
    """
    Коригує норми внесення NPK залежно від рівня опадів.
    """
    if precipitation < 450:  # Посушливі регіони
        n *= 0.9  # Зменшуємо азот на 10%
        p *= 1.1  # Збільшуємо фосфор на 10%
        k *= 1.1  # Збільшуємо калій на 10%
    elif precipitation > 700:  # Вологі регіони
        n *= 1.1  # Збільшуємо азот на 10%
        k *= 1.1  # Збільшуємо калій на 10%
        # Фосфор залишаємо без змін
...
# Отримуємо опади з даних користувача
precipitation = user_data.get('precipitation', 550)  # Якщо не задано, беремо середнє значення

# Коригуємо норми добрив на основі опадів
n, p, k = adjust_fertilizer_rates(n, p, k, precipitation)
