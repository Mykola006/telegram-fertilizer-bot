...
# Отримуємо середньорічні опади для регіону
precipitation = region_precipitation.get(text, 550)  # Якщо регіону немає, беремо середнє значення
await state.update_data(precipitation=precipitation)
...
# Коригування норм добрив на основі середньорічних опадів
precipitation = data.get('precipitation', 550)

if precipitation < 450:  # Посушливі умови
    N_factor *= 0.9
    P_factor *= 0.9
    K_factor *= 0.9
elif precipitation > 800:  # Висока зволоженість
    N_factor *= 1.1
    P_factor *= 1.1
    K_factor *= 1.1
